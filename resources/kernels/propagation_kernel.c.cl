...
inline floating_t mediumLayerBoundary(int layer) {
  return (convert_floating_t(layer) * ((floating_t)MEDIUM_LAYER_THICKNESS)) +
         (floating_t)MEDIUM_LAYER_BOTTOM_POS;
}

void scatterDirectionByAngle(floating_t cosa, floating_t sina,
                             floating4_t *direction, floating_t randomNumber) {
...
// randomize direction of scattering (rotation around old direction axis)
  ...
  const floating_t sinth =
      my_sqrt(max(ZERO, ONE - (*direction).z * (*direction).z));
  ...
  if (sinth > 0.f) { // Current direction not vertical, so rotate
    ...
    (*direction).x =
        oldDir.x * cosa -
        my_divide((oldDir.y * cosb + oldDir.z * oldDir.x * sinb) * sina, sinth);
    (*direction).y =
        oldDir.y * cosa +
        my_divide((oldDir.x * cosb - oldDir.z * oldDir.y * sinb) * sina, sinth);
  ...
  {
    const floating_t recip_length = my_rsqrt(
...
inline void createPhotonFromTrack(struct I3CLSimStep *step,
                                  const floating4_t stepDir, RNG_ARGS,
                                  floating4_t *photonPosAndTime,
                                  floating4_t *photonDirAndWlen) {
  floating_t shiftMultiplied =
      step->dirAndLengthAndBeta.z * RNG_CALL_UNIFORM_CO;
  floating_t inverseParticleSpeed =
      my_recip(speedOfLight * step->dirAndLengthAndBeta.w);
  ...
  *photonPosAndTime = (floating4_t)(
  ...
  if (step->sourceType == 0) {
    ...
    const floating_t cosCherenkov =
        min(ONE, my_recip(step->dirAndLengthAndBeta.w *
                          getPhaseRefIndex(layer, wavelength))); // cos theta =
                                                                 // 1/(beta*n)
...
// printf("endgen.\n");
    ...
    const floating_t wavelength =
        generateWavelength(convert_uint(step->sourceType), RNG_ARGS_TO_CALL);
...
inline float2 sphDirFromCar(double4 carDir) {
  ...
  const double r_inv =
      my_rsqrt(carDir.x * carDir.x + carDir.y * carDir.y + carDir.z * carDir.z);
...
inline void saveHit(const floating4_t photonPosAndTime,
                    const floating4_t photonDirAndWlen,
                    const floating_t thisStepLength, floating_t inv_groupvel,
                    floating_t photonTotalPathLength, uint photonNumScatters,
                    ...
                    const struct I3CLSimStep *step, unsigned short hitOnString,
                    unsigned short hitOnDom, __global uint *hitIndex,
                    uint maxHitIndex,
                    __global struct I3CLSimPhoton *outputPhotons
#ifdef SAVE_PHOTON_HISTORY
                    ,
                    __global float4 *photonHistory, float4 *currentPhotonHistory
#endif
                    ) {
  ...
  if (myIndex < maxHitIndex) {
    ...
    outputPhotons[myIndex].posAndTime =
        (float4)(photonPosAndTime.x + thisStepLength * photonDirAndWlen.x,
...
#ifdef DOUBLE_PRECISION
    outputPhotons[myIndex].startPosAndTime =
        (float4)(photonStartPosAndTime.x, photonStartPosAndTime.y,
                 photonStartPosAndTime.z, photonStartPosAndTime.w);
#else
...
#ifdef PRINTF_ENABLED
    dbg_printf(
        "     -> stored photon: p=(%f,%f,%f), d=(%f,%f), t=%f, wlen=%fnm\n",
...
__kernel void propKernel(
...
#endif
    __global ulong *MWC_RNG_x, __global uint *MWC_RNG_a) {
  ...
  __local unsigned short geoLayerToOMNumIndexPerStringSetLocal
      [GEO_geoLayerToOMNumIndexPerStringSet_BUFFER_SIZE];

  // copy the geo data to our local memory (this is done by a whole work group
  // in parallel)
  event_t copyFinishedEvent = async_work_group_copy(
      geoLayerToOMNumIndexPerStringSetLocal, geoLayerToOMNumIndexPerStringSet,
      (size_t)GEO_geoLayerToOMNumIndexPerStringSet_BUFFER_SIZE, 0);
...
// barrier(CLK_LOCAL_MEM_FENCE);
  ...
  floating4_t stepDir;
  {
    ...
    stepDir =
        (floating4_t)(rho * my_cos(step.dirAndLengthAndBeta.y), // rho*cos(phi)
  ...
  dbg_printf("Step at: p=(%f,%f,%f), d=(%f,%f,%f), t=%f, l=%f, N=%u\n",
             step.posAndTime.x, step.posAndTime.y, step.posAndTime.z, stepDir.x,
             stepDir.y, stepDir.z, step.posAndTime.w,
             step.dirAndLengthAndBeta.z, step.numPhotons);
#endif
...
#ifdef PRINTF_ENABLED
      dbg_printf("   created photon %u at: p=(%f,%f,%f), d=(%f,%f,%f), t=%f, "
                 "wlen=%fnm\n",
                 photonsLeftToPropagate - step.numPhotons, photonPosAndTime.x,
...
#ifdef getTiltZShift_IS_CONSTANT
      currentPhotonLayer = min(
          max(findLayerForGivenZPos(photonPosAndTime.z), 0), MEDIUM_LAYERS - 1);
#endif
...
#define effective_z (photonPosAndTime.z - getTiltZShift_IS_CONSTANT)
      ...
      // propagation step. Usually the factor is 1 and thus has no effect, but
      // it
...
#ifdef PRINTF_ENABLED
      dbg_printf(
          "    . colission detected, step limited to thisStepLength=%f!\n",
          distancePropagated);
    ...
    if (abs_lens_left < EPSILON) {
      ...
      if (RNG_CALL_UNIFORM_CO < SAVE_ALL_PHOTONS_PRESCALE) {
        saveHit(photonPosAndTime, photonDirAndWlen,
                ...
                &step, 0, // string id (not used in this case)
...
// calculate a new direction
#ifdef PRINTF_ENABLED
      dbg_printf(
          "   - photon is not yet absorbed (abs_len_left=%f)! Scattering!\n",
          abs_lens_left);
#endif
      ...
      scatterDirectionByAngle(cosScatAngle, sinScatAngle, &photonDirAndWlen,
                              RNG_CALL_UNIFORM_CO);
